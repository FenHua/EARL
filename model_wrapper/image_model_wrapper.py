...
# Initial perturbation generator
class TentativePerturbationGenerator():

    def __init__(self, extractors, part_size=100, preprocess=True, device=0):
        self.r = None
        self.extractors = extractors    # classifier
        self.part_size = part_size      # part_size
        self.preprocess = preprocess    # whether the input needs to be processed (default true)
        self.device = device            # running equipment

    # set the targeted attack parameters
    def set_targeted_params(self, target_vid, random_mask=1.):
        self.target = True                    # targeted attack
        self.random_mask = random_mask        # random mask
        self.target_feature = []              # features
        with torch.no_grad():
            ...
            if self.preprocess:
                ...
                std = torch.tensor([0.229, 0.224, 0.225], dtype=torch.float32, device=target_vid.get_device())[None, :,
                      None, None]
                target_vid = target_vid.sub_(mean).div_(std)  # normalize
            for extractor in self.extractors:
                outputs = extractor(target_vid)               # output features
                self.target_feature.append(outputs[0].view((outputs[0].size(0), -1)))

    # set untargeted attack parameters
    def set_untargeted_params(self, ori_video, random_mask=1., translate=0., scale=1.):
        self.target = False                  # un-targeted
        self.translate = translate           # translate
        self.scale = scale                   # resize scale
        self.random_mask = random_mask       # random mask
        self.target_feature = []             # extraction features
        with torch.no_grad():
            ori_video = ori_video.clone().cuda(self.device)
            for extractor in self.extractors:
                outputs = extractor(ori_video)                 # features
                output_size = outputs[0].size()                # length
                del outputs
                r = torch.randn(output_size, device=self.device) * self.scale + self.translate  # random
                r = torch.where(r >= 0, r, -r)                 # clip
                self.target_feature.append(r.view((output_size[0], -1)))

    # backpropagation update perturbations
    def backpropagate2frames(self, part_vid, start_idx, end_idx, random):
        part_vid.requires_grad = True      # gradient information
        ...
        for idx, extractor in enumerate(self.extractors):
            perturb_loss = 0
            o = extractor(processed_vid)[0]   # classification result
            o = o.view((o.size(0), -1))
            if self.target:
                if random:
                    # random mask loss
                ...
                perturb_loss += nn.MSELoss(reduction='mean')(o, r)  # un-targeted attack loss
            # print(perturb_loss.item())
        perturb_loss.backward()  # back propagation
        extractor.zero_grad()    # feature extraction network gradient reset
        sign_grad = torch.sign(part_vid.grad)  # vid gradient sign
        return sign_grad

    # Get an initial perturbations
    def create_adv_directions(self, vid, random=True):
        # video shape: [num_frames, c, w, h]
        vid = vid.clone().cuda(self.device)
        assert hasattr(self, 'target'), 'Error, AdvDirectionCreator\' mode unset'
        start_idx = 0
        adv_directions = []           # initial adversary
        part_size = self.part_size    # part_size
        while start_idx < vid.size(0):
            ...
            start_idx += part_size    # counter
        adv_directions = torch.cat(adv_directions, 0)  # concat
